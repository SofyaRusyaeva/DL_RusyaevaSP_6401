...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            layer_idx = i + 1
            d_in = all_dims[i]
            d_out = all_dims[layer_idx]
            
            self.params[f'W{layer_idx}'] = np.random.normal(0, weight_scale, (d_in, d_out))
            self.params[f'b{layer_idx}'] = np.zeros(d_out)
            
            if self.normalization == 'batchnorm' and layer_idx < self.num_layers:
                self.params[f'gamma{layer_idx}'] = np.ones(d_out)
                self.params[f'beta{layer_idx}'] = np.zeros(d_out)
        ...
        for i in range(self.num_layers - 1, 0, -1):
            ...
            if self.normalization == 'batchnorm':
                dout, grads[f'gamma{i}'], grads[f'beta{i}'] = \
                    batchnorm_backward(dout, bn_cache)
